...
class Vision:
    def __init__(self):
        self.lower_green = np.array([60, 177, 177])
        self.upper_green = np.array([160, 255, 255])
        self.lower_white = np.array([0, 0, 190])
        self.upper_white = np.array([360, 60, 255])
    ...
    def trackRetroTape(self, fieldOfView, screenWidth):
        ...
        pairs = np.array([[]])
        files = os.listdir(
            "C:/Users/User1/Desktop/Vision Images/Retro Tape/Low Exposure/"
        )
        index = 0 
    ...
    def trackGroundTape(self, fieldOfView, screenWidth):
        index = 0
